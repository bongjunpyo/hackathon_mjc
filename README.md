...
| 화면 | 설명 |
|---|---|
| 로드맵 스튜디오 | **2D 트랙(기찻길)** — 학기가 역, 자격증·이력서가 분기. 캐릭터가 트랙을 걸어 이동하고, 종착역에서 "졸업요건 충족 ✓" 배지 |
| 입력 | 학과 → 학년/학기 → 이수 과목 체크 → 목표 직무 (**미정 선택 가능** — 학과 데이터 기반 추천) |
| 학과 탐색 | 34개 학과 카드 → 상세(커리큘럼·자격증·진로) → "이 학과로 내 로드맵 그리기" |
| 리포트 | 학교용 — 직무 커버리지 · 자격증 결손 진단 |
| 로그인 | 학번 기반 (게스트 모드 지원 — 로그인 없이 전 기능 데모 가능) |
