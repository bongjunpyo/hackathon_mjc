...
그런데 학생이 손에 쥐는 건 교육과정표 PDF 한 장이다. *"네트워크 엔지니어가 되려면 어느 학기에 뭘 듣고 CCNA를 언제 따야 하는가"* 는 아무도 알려주지 않는다.
...
검증기는 LLM을 쓰지 않는다. 3년제 110학점(교양 10 · 전공 66 · 6학기) / 2년제 75학점(교양 6 · 전공 45 · 4학기)을 결정론적으로 검사하고, 미달 항목마다 `{rule, label, required, actual, shortfall, fix}`를 낸다. 이 `fix` 문구가 그대로 재생성 프롬프트가 되고, 동시에 화면에 표시된다.

이미 이수한 학기·학점을 함께 계산하므로 2·3학년 재학생도 남은 학기만으로 로드맵을 받는다. 테스트 12개로 2년제/3년제 규칙과 이수분 합산을 검증한다.
...
cd server
uv sync
uv run pytest          # 졸업요건 검증기 12개 테스트
```
...
검증 과정에서 잡은 오류는 `AI_USAGE.md`에 실패 사례로 남겼다. 그중 하나: 자격증 결손 진단이 커뮤니케이션디자인과에서 "25/25 결손"으로 나왔는데, 원본을 대조해 보니 진짜 결손이 아니라 **키워드 사전에 디자인 자격증이 없어서**였다. 측정값이 그럴듯해 보여도 방법론 결함일 수 있다는 사례다 — 판정 불가를 별도 분류로 분리했다.
