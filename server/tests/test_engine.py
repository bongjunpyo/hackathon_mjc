...
LLM은 18~68초가 걸리고 API 키가 필요하다. 키가 없거나 호출이 실패해도 로드맵은
나와야 한다. 대신 무엇이 돌았는지 `engine`으로 밝힌다 — 폴백이 조용히 일어나면
규칙 코드 결과를 LLM 결과로 오인한다.
"""
...
def test_env파일의_키만_있어도_LLM을_쓴다(client, monkeypatch, tmp_path):
    """pick_generator가 os.getenv를 보는데 .env는 agent.generate 안에서 로드됐다.
    그래서 .env에 키가 있어도 규칙 플래너가 돌았다 — 실제로 그랬다."""
