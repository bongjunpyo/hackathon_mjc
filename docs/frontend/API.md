...
### 지연 (8/7 02시 엔진 교체 반영 — PR 참조)

| 엔진 | 응답 | 프론트 대응 |
|---|---|---|
| **rule (기본)** | **즉시 (~0.1초)** | 실데이터 기반 결정론 추천. GENERATING 연출은 짧은 트랙 드로잉 트랜지션으로 충분 |
| llm (`MJC_ENGINE=llm`일 때만) | 27~120초 | 비교 데모용 보조. 이 모드를 켤 때만 150초 timeout(`AbortController`) + 단계 연출 필요 |

### 직무 미정 모드 (신규)

`target_job`을 **생략(또는 빈 문자열)**하면 400이 아니라, 라벨별 배정 학점 1위 직무를 추천해 그걸로 짠다. 응답에 세 필드가 추가된다:

```json
{
  "target_job": "시스템응용SW개발엔지니어",
  "job_recommended": true,
  "recommended_jobs": [
    { "job": "시스템응용SW개발엔지니어", "credits": 42, "courses": 14 },
    { "job": "시스템관리·운용엔지니어", "credits": 36, "courses": 12 }
  ]
}
```

- 화면: "직무 미정 — **{1위}**(으)로 추천 로드맵을 그렸습니다" 배너 + 나머지 후보를 칩으로. 칩 클릭 → 그 직무로 재생성 (즉시라 부담 없음).
- 추천 근거는 라벨별 배정 학점 합 — 트랙 B 커버리지와 같은 계산이라 "왜 이 직무냐"에 학과 데이터로 답할 수 있다.
- 직무 드롭다운에 **"아직 못 정했어요"** 항목을 첫 번째로 추가할 것.
