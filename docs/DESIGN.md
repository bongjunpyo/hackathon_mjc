...
| AI 활용 역량 | 20 | 로드맵 에이전트 + 검증기 자기수정 루프. **위임 범위를 측정으로 정했다** — 헤더 일관성을 확인하고 추출은 코드로, LLM은 폴백으로 (폴백 발동 0회) |
...
3. **트랙 B (학교용)**: 교육과정 진단 리포트 (화면 1~2개) — 아래 §5 트랙 B 참조
...
[DB]    courses.json (전 학과) + certificates + careers + 졸업요건 룰
   ↓
[매핑]  비고 열의 인재양성유형 = 학교가 부여한 과목↔직무 라벨 (34/35 학과 보유)
        + 학과 소개 페이지의 NCS 직무명 = 학교 공식 NCS 매핑
   ↓                                    ↓
...
**교양 처리 (팀 결정 2026-08-06)** — 교과과정표는 전공 전용 문서라 교양 과목이 실리지 않는다.
...
외부 데이터 없이 **학교 자체 데이터 간 정합성**만으로 성립하는 진단이다. NCS 원문
대조는 v1 범위 밖 — 대신 학교가 직접 부여한 라벨을 축으로 쓴다.
...
**자격증 판정은 3분류다: 대응 있음 / 대응 없음 / 판정 불가.** 키워드 사전이 IT·공학
위주라 디자인·어학 자격증은 애초에 판정할 수 없다. 이를 "대응 없음"으로 세면 없는
결손을 지어내게 되므로 `unevaluated`로 분리한다 — 리포트에도 그대로 표기한다.
...
| 담당 | 역할 | 산출물 |
|---|---|---|
| **P1 효민** (백엔드·파이썬 + 기획·문서·발표) | 수집 + LLM 추출 파이프라인 / README·PPT·발표(3~4분) 겸임 | courses.json 전 학과, 품질 지표 표, 문서 전부 |
| **P2 동제** (백엔드·API) | 검증기 + 로드맵 에이전트 + 정합도 계산 + JWT 인증·DB | API 서버 |
| **P3 준표** (프론트) | 화면 7종 (랜딩·로드맵 스튜디오·3D 걷기·탐색·상세·리포트·로그인) | React 앱 |
